...
if [ -z "$(ls 'data')" ] ; then
    ...
    mysqladmin --defaults-file="config/my.cnf"   \
               --user="root"                     \
               --socket="$(pwd)/run/mysqld.sock" \
               --host="$BLOG_DATABASE_HOST"      \
               --port="$BLOG_DATABASE_PORT"      \
               password "$BLOG_DATABASE_ROOT_PASSWORD" &> log/mysqladmin.error.log
else
    startDatabaseServer
fi
...
-- blog
CREATE USER IF NOT EXISTS '$BLOG_DATABASE_USER'@'localhost' IDENTIFIED BY '$BLOG_DATABASE_PASSWORD';
CREATE USER IF NOT EXISTS '$BLOG_DATABASE_USER'@'%' IDENTIFIED BY '$BLOG_DATABASE_PASSWORD';
CREATE DATABASE IF NOT EXISTS \`$BLOG_DATABASE_NAME\` DEFAULT CHARACTER SET 'utf8';
GRANT ALL PRIVILEGES ON \`${BLOG_DATABASE_NAME//_/\\_}\`.* TO '$BLOG_DATABASE_USER'@'localhost';
GRANT ALL PRIVILEGES ON \`${BLOG_DATABASE_NAME//_/\\_}\`.* TO '$BLOG_DATABASE_USER'@'%';
SQL
...
mysql --defaults-file="config/my.cnf"           \
      --user="root"                             \
      --password="$BLOG_DATABASE_ROOT_PASSWORD" \
      --socket="$(pwd)/run/mysqld.sock"         \
      --host="$BLOG_DATABASE_HOST"              \
      --port="$BLOG_DATABASE_PORT"              \
